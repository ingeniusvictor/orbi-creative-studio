...
test('P1C53 cleanup removes destination and every transactional download artifact', async () => {
    const { removeDownloadArtifacts } = require('../electron/lib/localInferenceDownload');
    await withTempDir(async (dir) => {
        const destination = path.join(dir, 'model.bin');
        for (const suffix of ['', '.part', '.part.meta.json', '.part.fresh']) {
            fs.writeFileSync(destination + suffix, 'artifact');
        }

        removeDownloadArtifacts(destination);

        for (const suffix of ['', '.part', '.part.meta.json', '.part.fresh']) {
            assert.equal(fs.existsSync(destination + suffix), false);
        }
    });
});

test('P1C53 local inference delegates downloads and model deletion to the verified downloader', () => {
    const source = fs.readFileSync(
        path.resolve(__dirname, '..', 'electron', 'lib', 'localInference.js'),
        'utf8'
    );

    assert.ok(source.includes("require('./localInferenceDownload')"));
    assert.equal(source.includes("function downloadFile(url, destPath, onProgress)"), false);
    assert.ok(source.includes('removeDownloadArtifacts(filePath)'));
    assert.ok(source.includes('removeDownloadArtifacts(zipPath)'));
});
