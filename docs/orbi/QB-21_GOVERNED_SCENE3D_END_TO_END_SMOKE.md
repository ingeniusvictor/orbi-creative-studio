...
QB-21 v2 is rebuilt on QB-20 v2 and inherits the complete QB-12 → QB-20 dependency chain.
...
**QB-21 v2 status: IMPLEMENTATION CANDIDATE — ready for cumulative CI and live cross-runtime validation.**


## Rebuild note

QB-21 v2 starts from the current hardened QB-20 v2 stack. It adds validation/evidence only:

- strict provider-free dry-run evidence validation,
- reviewed recipe code SHA identity,
- just-in-time provider-free dry-run revalidation before execution,
- code-change denial before side effect,
- cumulative authority regressions,
- live governed end-to-end smoke.

No additional renderer/provider authority is introduced and no canonical branch is modified.
